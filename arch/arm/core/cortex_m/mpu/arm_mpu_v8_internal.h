	...
	bool do_sanity_check)
{
	...
	for (i = 0; i < regions_num; i++) {
		...
		if ((u_reg_index == -EINVAL) ||
			...
			__ASSERT(0,	"Invalid underlying region index %u",
				u_reg_index);
			return -EINVAL;
		}
 ...
 * where dynamic region programming is allowed.
 */
static void _mpu_mark_areas_for_dynamic_regions(
		...
		const u8_t dyn_region_areas_num)
{
	...
	for (int i = 0; i < dyn_region_areas_num; i++) {
		...
		__ASSERT(dyn_reg_info[i].index != -EINVAL,
			"Invalid index for dynamic region area\n");
		__ASSERT(dyn_reg_info[i].index < static_regions_num,
			"Invalid index for dynamic region area %u\n",
			dyn_reg_info[i].index);
